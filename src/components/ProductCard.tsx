        ...
        <div className="flex gap-3 p-4 md:p-5 min-h-[140px]">
          <div className="flex-1 flex flex-col justify-between min-w-0">
            <div className="space-y-1.5">
              <h3 className="text-[16px] md:text-[18px] font-extrabold text-white leading-snug line-clamp-2 tracking-[-0.01em]">{name}</h3>
              ...
              {hasPromotion && (
                <>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {promotionName && <span className="bg-primary text-primary-foreground text-[11px] font-bold px-2 py-0.5 rounded">{promotionName}</span>}
                  </div>
                  {timeRemaining && (
                    <div className={cn("inline-flex items-center gap-1.5 bg-[#ff8c00]/15 text-[#ff8c00] text-[11px] font-semibold px-2.5 py-1 rounded-full", getTimerPulseClass())}>
